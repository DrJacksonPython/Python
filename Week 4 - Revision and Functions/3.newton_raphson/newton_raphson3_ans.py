...
def NewtonRaphson(Fn, DiffFn, startValue, tol):
    count = 1#create a variable called count - give it a sensible start value
    xOld = startValue - 100 * tol # why have I initialised xOld in this way?
    xNew = startValue#what should the value of xNew be?
    while abs(xNew - xOld) > tol and count < 20: #you need to check if 20 iterations have occured
        xOld = xNew #you need to assign xOld a value - what is it?
        ...
        count += 1 # what do you need to do to count here?
    return xNew # what should you return?
